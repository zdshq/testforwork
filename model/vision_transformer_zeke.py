...
__all__ = ['VisionTransformer_zeke']  # model_registry will add each entrypoint fn to this
...
class MoeMlp(nn.Module):
    def __init__(self, 
        ...
        self.experts = nn.ModuleList([
            nn.Sequential(
                nn.Linear(in_features, in_features * moe_mlp_ratio),
                act_layer(),
                nn.Linear(in_features * moe_mlp_ratio, out_features),
            )
...
class Block(nn.Module):
    def __init__(
            ...
            # moe para
            moe_layer: Type[nn.Module] = MoeLayer,
            moe_idx: List[int] = [6, 7, 8, 9, 10, 11],
            per_expert_slots: int = 1,
            per_layer_experts: int = 4,
            moe_mlp_ratio: int = 4,
            block_idx: int = None,  # 新增参数，表示当前块的索引
    ) -> None:
        super().__init__()
        ...
        # 根据当前块索引决定使用标准MLP还是MoE
        use_moe = block_idx is not None and block_idx in moe_idx
...
class VisionTransformer_zeke(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            # moe parameters
            num_experts: int = 4,
            slots_per_expert: int = 1,                          # 每个专家的槽位数量
            moe_layer_idx: List[int] = [6, 7, 8, 9, 10, 11],           # 在哪些层使用MOE
    ) -> None:
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[  # * 是解包的意思，把列表中的每个元素解包成单独的参数
            block_fn(
                dim=embed_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_ratio,
                ...
                mlp_layer=mlp_layer,
                moe_layer=MoeLayer,
                moe_idx = moe_layer_idx,
                per_expert_slots=slots_per_expert,
                per_layer_experts=num_experts,
...
@torch.no_grad()
def _load_weights(model: VisionTransformer_zeke, checkpoint_path: str, prefix: str = '', load_bfloat16: bool = False) -> None:
...
def _create_vision_transformer(variant: str, pretrained: bool = False, **kwargs) -> VisionTransformer_zeke:
